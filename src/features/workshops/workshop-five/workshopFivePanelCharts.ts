...
import type {WorkshopFiveExternalRow, WorkshopFiveLeakRow} from './workshopFiveSidePanelBindings';
...
export function createWorkshopFiveLeakBarOption(rows: WorkshopFiveLeakRow[]): EChartsOption {
...
export function createWorkshopFiveFlowLiquidFillOption(instantRaw: number, totalRaw: number): EChartsOption {
...
export function createWorkshopFiveLoadingBarOption(instant: number, total: number): EChartsOption {
    const rows = [
...
export function createWorkshopFiveTemperatureGaugeOption(value: number): EChartsOption {
...
export function createWorkshopFiveExternalBarOption(rows: WorkshopFiveExternalRow[]): EChartsOption {
    const values = rows.map((row) => (Number.isFinite(row.value) ? row.value : 0));
