...
import type {WorkshopTwoExternalRow, WorkshopTwoLeakRow} from './workshopTwoSidePanelBindings';
...
export function createWorkshopTwoLeakBarOption(rows: WorkshopTwoLeakRow[]): EChartsOption {
...
export function createWorkshopTwoFlowLiquidFillOption(instantRaw: number, totalRaw: number): EChartsOption {
...
export function createWorkshopTwoLoadingBarOption(instant: number, total: number): EChartsOption {
    const rows = [
...
export function createWorkshopTwoTemperatureGaugeOption(value: number): EChartsOption {
...
export function createWorkshopTwoExternalBarOption(rows: WorkshopTwoExternalRow[]): EChartsOption {
    const values = rows.map((row) => (Number.isFinite(row.value) ? row.value : 0));
