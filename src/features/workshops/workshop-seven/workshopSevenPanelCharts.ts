...
import type {WorkshopSevenExternalRow, WorkshopSevenLeakRow} from './workshopSevenSidePanelBindings';
...
export function createWorkshopSevenLeakBarOption(rows: WorkshopSevenLeakRow[]): EChartsOption {
...
export function createWorkshopSevenFlowLiquidFillOption(instantRaw: number, totalRaw: number): EChartsOption {
...
export function createWorkshopSevenLoadingBarOption(instant: number, total: number): EChartsOption {
    const rows = [
...
export function createWorkshopSevenTemperatureGaugeOption(value: number): EChartsOption {
...
export function createWorkshopSevenExternalBarOption(rows: WorkshopSevenExternalRow[]): EChartsOption {
    const values = rows.map((row) => (Number.isFinite(row.value) ? row.value : 0));
