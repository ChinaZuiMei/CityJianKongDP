...
import type {WorkshopSixExternalRow, WorkshopSixLeakRow} from './workshopSixSidePanelBindings';
...
export function createWorkshopSixLeakBarOption(rows: WorkshopSixLeakRow[]): EChartsOption {
...
export function createWorkshopSixTemperatureGaugeOption(value: number): EChartsOption {
...
export function createWorkshopSixExternalBarOption(rows: WorkshopSixExternalRow[]): EChartsOption {
    const values = rows.map((row) => (Number.isFinite(row.value) ? row.value : 0));
