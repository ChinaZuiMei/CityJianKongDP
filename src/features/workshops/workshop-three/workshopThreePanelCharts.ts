...
import type {WorkshopThreeExternalRow, WorkshopThreeLeakRow} from './workshopThreeSidePanelBindings';
...
export function createWorkshopThreeLevelBarOption(rows: WorkshopThreeLeakRow[]): EChartsOption {
    return createWorkshopThreeMetricBarOption(rows, 'm', 1);
}

function createWorkshopThreeMetricBarOption(
    rows: WorkshopThreeLeakRow[],
    unit: string,
    minBackground: number,
): EChartsOption {
    const labels = rows.map((row) => row.label);
    const values = rows.map((row) => (Number.isFinite(row.value) ? row.value : 0));
    const maxValue = Math.max(...values.map((v) => Math.abs(v)), minBackground);
    ...
    return {
        ...
        tooltip: {
            ...
            formatter: (params: unknown) => {
                ...
                return `${labels[idx]}<br/>数值 : ${formatMetricValue(values[idx])} ${unit}`;
            },
        },
        xAxis: {show: false, type: 'value', max: unit === 'ppm' ? maxValue : undefined},
        yAxis: [
            {
                type: 'category',
                inverse: true,
            ...
            },
            {
                ...
                axisLabel: {
                    color: '#ffffff',
                    fontSize: 12,
                    formatter: (_value: string, index: number) => `${formatMetricValue(values[index])} ${unit}`,
                },
                data: values,
            },
        ],
        series: [
            {
                ...
                barWidth: 8,
                data: unit === 'ppm' ? values.map((v) => Math.max(v, 0)) : values,
            },
            {
                name: '背景',
                type: 'bar',
...
export function createWorkshopThreeLeakBarOption(rows: WorkshopThreeLeakRow[]): EChartsOption {
    return createWorkshopThreeMetricBarOption(rows, 'ppm', 0.2);
}

export function createWorkshopThreeFlowLiquidFillOption(instantRaw: number, totalRaw: number): EChartsOption {
...
export function createWorkshopThreeLoadingBarOption(instant: number, total: number): EChartsOption {
    const rows = [
...
export function createWorkshopThreeTemperatureGaugeOption(value: number): EChartsOption {
...
export function createWorkshopThreeExternalBarOption(rows: WorkshopThreeExternalRow[]): EChartsOption {
    const values = rows.map((row) => (Number.isFinite(row.value) ? row.value : 0));
