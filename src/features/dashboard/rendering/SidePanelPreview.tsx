        ...
        <PanelShell title={schema.title} subtitle={schema.subtitle}>
            <div className="space-y-3">
                {schema.items.map((item) => {
                    ...
                    return (
                        <div key={item.id}
                             className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-lg border border-sky-300/10 bg-slate-950/16 px-3 py-2">
                            <div className="text-sm font-bold text-slate-200">{item.label}</div>
...
function renderMonitorTable(schema: MonitorTablePanelSchema, state: PreviewState) {
    ...
    return (
        <PanelShell title={schema.title} subtitle={schema.subtitle}>
            <div
                className="grid grid-cols-4 gap-2 border-b border-sky-300/10 pb-2 text-[11px] font-black tracking-[0.14em] text-slate-400 uppercase">
            ...
            <div className="mt-3 space-y-2">
                {tableRows.map((row, index) => (
                    <div key={index}
                         className="grid grid-cols-4 gap-2 rounded-md border border-sky-300/8 bg-slate-950/16 px-2 py-2 text-sm text-slate-200">
                        {schema.columns.map((column) => (
                            <div key={column.id}
                                 className={column.id === 'name' ? 'font-bold' : 'text-right font-mono'}>
