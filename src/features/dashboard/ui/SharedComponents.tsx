        ...
        <div className="flex flex-col items-center gap-2">
            {isImageTank ? (
                <div className={cn("relative flex items-center justify-center transition-all", imageTankSizeClass)}>
                    {hasAlarm ? <div
                        className="absolute inset-0 z-10 rounded-[28px] bg-red-500/8 blur-xl animate-pulse pointer-events-none"/> : null}
                    <img
                        src={tankImage}
                        alt={label}
                        className={cn(
                ...
                <div
                    className={cn("relative flex h-40 w-24 flex-col-reverse overflow-hidden rounded-t-xl border-2 bg-transparent transition-all", hasAlarm ? "border-red-500 animate-pulse-border" : "panel-frame")}>
                    {hasAlarm &&
                        <div className="absolute inset-0 z-10 animate-pulse bg-red-500/10 pointer-events-none"/>}
                    <motion.div initial={{height: 0}} animate={{height: `${fillPercent}%`}}
                                className={cn(color, "w-full opacity-90 relative")}>
                    ...
                    <div
                        className="absolute right-1.5 top-4 bottom-0 w-2.5 flex flex-col justify-between py-2 opacity-50">
            ...
            <div
                ...
                <div className="flex items-center justify-center gap-2">
                    <div
                        className={cn("flex h-6 items-center text-base font-bold tracking-wide", hasAlarm ? "font-extrabold text-red-200" : "panel-title-glow")}>
                        {label}
                    </div>
