...
export const TankAreaScreen = ({
                                   data,
                                   alarmData,
                                   hideTankAlarmOverlay = false,
                               }: {
    data: ScadaData;
    alarmData: AlarmData;
    ...
    <div className="flex h-full w-full flex-col gap-2 overflow-visible px-2 py-1 xl:px-3 xl:py-2">
        <div className="grid min-h-0 grid-cols-4 gap-4 pb-2 overflow-visible">
            {[
                ...
                return (
                    <div key={tank.id}
                         className="relative flex min-h-0 flex-col items-center gap-1 overflow-visible px-1 py-0.5">
                        {!hideTankAlarmOverlay && alarmNames.length > 0 && (
                            <div
                                className="pointer-events-none absolute left-1/2 top-0 z-30 flex min-w-[110px] max-w-[170px] -translate-x-1/2 -translate-y-[92%] flex-col items-center gap-1 rounded-md border border-red-400/80 bg-slate-950/70 px-2 py-1 shadow-[0_0_12px_rgba(239,68,68,0.35)]">
                                {alarmNames.map((name, idx) => (
                                    <div key={idx}
                                         className="truncate text-center text-xs font-bold leading-5 text-red-100 xl:text-sm">
                                        {name}
                                    </div>
                                ))}
                            </div>
        ...
        <div className="grid grid-cols-3 gap-4 pt-1">
            {[
                ...
                return (
                    <div key={leak.id}
                         className="group flex min-h-[112px] flex-col items-center justify-start gap-1 overflow-visible px-2 py-1.5">
                        <div
                            className={cn("flex items-center gap-1 text-xs font-black uppercase tracking-[0.1em] xl:text-sm", isAlarm ? "text-red-200 animate-pulse" : "text-slate-100")}>
                            <AlertTriangle size={14}
                                           className={cn(isAlarm ? "text-red-400 animate-pulse" : "text-sky-300")}/> 盐酸泄漏 {leak.id}
                        </div>
                        <div
                            className={cn(
                        ...
                        <div className="flex min-h-[32px] flex-col items-center gap-1">
                            {alarmNames.map((name, idx) => (
                                <div key={idx}
                                     className="truncate px-2 py-0.5 text-center text-[10px] font-bold text-red-100 animate-pulse xl:text-[11px]">
                                    {name}
                                </div>
                            ))}
                        </div>
